...
const SchoolLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const items = [{ type: 'group' as const, label: '学校工作台', children: [
    { key: '/school/dashboard', icon: <HomeOutlined />, label: '成长概览' },
    { key: '/school/profile', icon: <UserOutlined />, label: '账号设置' },
  ] }]
  return <Layout className="app-shell"><Sider className="app-sider" width={248} trigger={null}>
    <div className="brand-lockup"><div className="brand-mark">M</div><div><strong>面试罗盘</strong><span>CAMPUS CONSOLE</span></div></div>
    <Menu className="app-menu" theme="dark" mode="inline" selectedKeys={[location.pathname]} items={items} onClick={({ key }) => navigate(key)} />
    <div className="sider-bottom"><div className="account-strip"><Avatar size={34} icon={<UserOutlined />} className="account-avatar" /><div className="account-name"><strong>{userInfo?.realName || userInfo?.username || '学校管理员'}</strong><span>学校端</span></div><Tooltip title="退出登录"><Button type="text" icon={<LogoutOutlined />} onClick={() => { logout(); navigate('/login') }} /></Tooltip></div></div>
