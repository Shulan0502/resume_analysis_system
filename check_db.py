...
print("角色表字段:", [col[0] for col in columns])
...
columns = cur.fetchall()
print("用户表字段:", [col[0] for col in columns])
...
users = cur.fetchall()
for user in users:
    print(f"ID: {user[0]}, 用户名: {user[1]}, 密码: {user[2]}, 邮箱: {user[3]}, 真实姓名: {user[4]}, 状态: {user[5]}, 角色ID: {user[6]}")
